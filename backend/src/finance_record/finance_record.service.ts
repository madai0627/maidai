...
export class FinanceRecordService {
  ...
  async list(q: QueryRecordDto) {
    ...
    if (q.minAmount != null || q.maxAmount != null) {
      ...
      if (q.minAmount != null)
        qb.andWhere('CAST(r.amount AS DECIMAL(12,2)) >= :min', {
          min: q.minAmount,
        });
      if (q.maxAmount != null)
        qb.andWhere('CAST(r.amount AS DECIMAL(12,2)) <= :max', {
          max: q.maxAmount,
        });
      qb.orderBy(order);
  ...
  async update(id: number, dto: UpdateRecordDto) {
    ...
    if (dto.amount !== undefined && dto.amount !== null) {
      updateData.amount = (dto.amount as any)?.toFixed
  ...
  async statsByDay(start?: string, end?: string) {
    ...
    const rows = await this.repo
      ...
      .addSelect(
        "SUM(CASE WHEN r.category = '收入' THEN CAST(r.amount AS DECIMAL(12,2)) ELSE 0 END)",
        'income',
      )
      .addSelect(
        "SUM(CASE WHEN r.category = '支出' THEN CAST(r.amount AS DECIMAL(12,2)) ELSE 0 END)",
        'expense',
      )
      .where('r.created_at BETWEEN :start AND :end', {
        start: startDate,
