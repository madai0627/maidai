  ...
  async seedInterview() {
    ...
    const items: Array<{
      ...
      },
      {
        ...
        optionA: '箭头函数的 this 在调用时动态绑定',
        optionB: '箭头函数没有自己的 this，取决于定义时外层作用域',
        optionC: 'call 可以改变箭头函数 this',
        optionD: 'bind 会立即执行函数',
        correctAnswer: 'B',
      },
      {
        categoryId: jsCat.id,
        content: '数组去重最合适的方式是：',
        optionA: 'arr.filter(Boolean)',
        optionB: 'new Set(arr) 配合展开',
        optionC: 'arr.map()',
        optionD: 'arr.forEach()',
        correctAnswer: 'B',
      },
      ...
      },
      {
        ...
        optionA: '直接修改 ref 的 value',
        optionB: '给 reactive 对象新增不存在的属性',
        optionC: '替换 reactive 对象为新对象',
        optionD: '修改 reactive 对象已有属性',
        correctAnswer: 'B',
      },
      {
        categoryId: vueCat.id,
        ...
        optionA: 'this.$router',
        optionB: 'useRouter()',
        optionC: 'inject("router")',
        optionD: 'getCurrentInstance().proxy.$router',
        correctAnswer: 'B',
      },
      {
        categoryId: webCat.id,
        content: '减少首屏白屏时间最有效的方式是：',
        optionA: '把所有 CSS 放在底部',
        optionB: '开启 HTTP/3',
        optionC: '对关键路径资源进行内联或预加载',
        optionD: '只用 setTimeout 延迟脚本',
        correctAnswer: 'C',
      },
      {
        categoryId: webCat.id,
        content: '关于防抖与节流，以下描述正确的是：',
        optionA: '防抖保证高频触发中按时间间隔执行',
        optionB: '节流在触发后 N ms 内忽略后续触发',
        optionC: '防抖适合输入框搜索，节流适合滚动监听',
        optionD: '两者等价',
        correctAnswer: 'C',
      },
    ];
    ...
    const existing = await this.questionRepo.find({
      select: { id: true, content: true } as any,
    });
    ...
    return {
      created: true,
      categories: [jsCat, vueCat, webCat].map((c) => ({
        id: c.id,
        name: c.name,
      })),
      added:
        items.length -
        existing.filter((q: any) => items.find((i) => i.content === q.content))
          .length,
    };
  }
    ...
    try {
      ...
      for (let i = 0; i < rows.length; i++) {
        ...
        try {
          ...
          const correctAnswer = row[headerMap['正确答案']]
            ?.toString()
            .trim()
            .toUpperCase();
          const difficulty =
            parseInt(row[headerMap['难度']]?.toString().trim()) || 1;
